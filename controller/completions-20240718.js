      ...
      completions.push(
        await req.db.sequelize.query(
          ...
          LEFT JOIN (
            ...
            FROM fas_coss_${com.year}.logcoss.jrn_list
            WHERE active = 'Y' AND depcode IN (${req.query.depcodes}) AND LEFT(attr, 1) = '0' AND attr <> '0999' AND code = '${com.sectionCode}'
    ...
    `SELECT ISNULL(coss.[row], com.[row]) AS [row], ${year} AS year, ISNULL(coss.[month], com.[month]) AS [month], ISNULL(coss.[amount], 0) AS coss_amount, ISNULL(com.[amount], 0) AS com_amount FROM (
      SELECT ${month} AS [month], [row], [amount] FROM (
        SELECT
          ${select}
        FROM (
          SELECT attr, ISNULL(debet, 0) - ISNULL(credit, 0) AS amount
          FROM fas_coss_${year}.logcoss.jrn_list
          WHERE active = 'Y' AND smonth = ${month} AND LEFT(ATTR, 1) = '0' AND Attr <> '0999' AND code = '${code}' AND depcode = ${cossDepcode} ${txt}
        ) jrn
      ) a
